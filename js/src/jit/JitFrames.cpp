...
GetPcScript(JSContext* cx, JSScript** scriptRes, jsbytecode** pcRes)
{
    ...
    if (it.isExitFrame()) {
        ...
        if (it.isExitFrameLayout<LazyLinkExitFrameLayout>()) {
            JitFrameLayout* layout = it.exitFrame()->as<LazyLinkExitFrameLayout>()->jsFrame();
            *scriptRes = ScriptFromCalleeToken(layout->calleeToken());
            if (pcRes)
                *pcRes = (*scriptRes)->code();
            return;
        }
#endif
        ++it;
