...
AppleVDADecoder::InitializeSession()
{
  ...
  if (rv != noErr) {
    ...
    const size_t extraDataLength = mExtraData->Length();
    const unsigned profile = extraDataLength > 1 ? extraData[1] : 0;
    const unsigned compatibility = extraDataLength > 2 ? extraData[2] : 0;
    const unsigned level = extraDataLength > 3 ? extraData[3] : 0;
    NS_WARNING(nsPrintfCString(
      "AppleVDADecoder: VDADecoderCreate failed with error %d (%s); "
      "%ux%u, %lu-byte avcC, profile %u, compatibility 0x%02x, level %u.",
      rv, VDAErrorName(rv), mPictureWidth, mPictureHeight,
      static_cast<unsigned long>(extraDataLength), profile, compatibility,
      level).get());
    return NS_ERROR_FAILURE;
  }
...
#ifndef MOZ_WIDGET_UIKIT
  // Output format type:
  // Firefox 48 used packed UYVY for IOSurface-backed VDA output.
